...
# MIT License
#
# For Minecraft servers running in Docker.
# For most convenience, run automatically with cron.

# Default Configuration 
CONTAINER_NAME="mc" # Name of the GNU container your Minecraft server is running in
SERVER_WORLD="/opt/minecraft/world/" # Server world directory
BACKUP_DIRECTORY="/opt/map_backup/" # Directory to save backups in
...
COMPRESSION_ALGORITHM="pigz" # Leave empty for no compression
...
while getopts 'a:cd:e:f:hi:l:m:o:p:qs:v' FLAG; do
  case $FLAG in
    ...
    h) echo "Minecraft Backup (by Nicolas Chan)"
       ...
       echo "-s    Minecraft server container name"
       echo "-v    Verbose mode"
       exit 0
       ;;
...
# Minecraft server container interface functions
message-players () {
  local MESSAGE=$1
...
if [[ "$ARCHIVE_SIZE" != "" && "$ARCHIVE_SIZE_BYTES" -gt 8 ]]; then
  ...
  message-players-error "Backup was not saved!" "Please notify the monkeys"
fi
